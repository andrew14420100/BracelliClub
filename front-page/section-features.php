<div class="front_page_section front_page_section_features<?php
	$sweat_scheme = sweat_get_theme_option( 'front_page_features_scheme' );
	if ( ! empty( $sweat_scheme ) && ! sweat_is_inherit( $sweat_scheme ) ) {
		echo ' scheme_' . esc_attr( $sweat_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( sweat_get_theme_option( 'front_page_features_paddings' ) );
	if ( sweat_get_theme_option( 'front_page_features_stack' ) ) {
		echo ' sc_stack_section_on';
	}
?>"
		<?php
		$sweat_css      = '';
		$sweat_bg_image = sweat_get_theme_option( 'front_page_features_bg_image' );
...
<?php
	// Add anchor
	$sweat_anchor_icon = sweat_get_theme_option( 'front_page_features_anchor_icon' );
	$sweat_anchor_text = sweat_get_theme_option( 'front_page_features_anchor_text' );
...
?>
	...
	<?php
	if ( sweat_get_theme_option( 'front_page_features_fullheight' ) ) {
	...
	?>
			...
			<?php
			$sweat_css      = '';
			$sweat_bg_mask  = sweat_get_theme_option( 'front_page_features_bg_mask' );
			$sweat_bg_color_type = sweat_get_theme_option( 'front_page_features_bg_color_type' );
			if ( 'custom' == $sweat_bg_color_type ) {
				$sweat_bg_color = sweat_get_theme_option( 'front_page_features_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_features_content_wrap content_wrap">
			<?php
			// Caption
			$sweat_caption = sweat_get_theme_option( 'front_page_features_caption' );
			if ( ! empty( $sweat_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_features_caption front_page_block_<?php echo ! empty( $sweat_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( $sweat_caption, 'sweat_kses_content' ); ?></h2>
				<?php
			}

			// Description (text)
			$sweat_description = sweat_get_theme_option( 'front_page_features_description' );
			if ( ! empty( $sweat_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_features_description front_page_block_<?php echo ! empty( $sweat_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $sweat_description ), 'sweat_kses_content' ); ?></div>
				<?php
			}
			...
			<div class="front_page_section_output front_page_section_features_output">
				...
				if ( is_active_sidebar( 'front_page_features_widgets' ) ) {
					...
					if ( ! sweat_exists_trx_addons() ) {
						...
						sweat_customizer_need_widgets_message( 'front_page_features_caption', 'ThemeREX Addons - Services' );
					}
				}
				?>
			</div>
