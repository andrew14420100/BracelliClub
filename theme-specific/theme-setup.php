...
if ( ! defined( 'SWEAT_THEME_FREE' ) ) {
	define( 'SWEAT_THEME_FREE', false );
}
if ( ! defined( 'SWEAT_THEME_FREE_WP' ) ) {
	define( 'SWEAT_THEME_FREE_WP', false );
}

// If this theme support external updates
if ( ! defined( 'SWEAT_THEME_ALLOW_UPDATE' ) ) {
	define( 'SWEAT_THEME_ALLOW_UPDATE', true );
}

// If this theme uses skins
if ( ! defined( 'SWEAT_ALLOW_SKINS' ) ) {
	define( 'SWEAT_ALLOW_SKINS', true );
}
if ( ! defined( 'SWEAT_DEFAULT_SKIN' ) ) {
	define( 'SWEAT_DEFAULT_SKIN', 'default' );
}
if ( ! defined( 'SWEAT_REMEMBER_SKIN' ) ) {
	define( 'SWEAT_REMEMBER_SKIN', false );
}
...
if ( ! function_exists( 'sweat_get_theme_pro_key' ) ) {
	/**
	 ...
	 */
	function sweat_get_theme_pro_key() {
		$theme_pro_key = get_option( sprintf( 'purchase_code_src_%s', get_template() ) );
		if ( empty( $theme_pro_key ) ) {
			$theme_pro_key = sweat_storage_get( 'theme_pro_key' );
		}
		return $theme_pro_key;
	}
}

if ( ! function_exists( 'sweat_trx_updater_get_theme_info' ) ) {
	add_filter( 'trx_updater_filter_get_theme_info', 'sweat_trx_updater_get_theme_info' );
	/**
	 ...
	 */
	function sweat_trx_updater_get_theme_info( $info ) {
		$engine = sweat_storage_get( 'engine' );
...
if ( ! function_exists( 'sweat_original_theme_slug' ) ) {
	add_filter( 'sweat_filter_original_theme_slug', 'sweat_original_theme_slug' );
	/**
...
if ( ! function_exists( 'sweat_importer_set_options' ) ) {
	add_filter( 'trx_addons_filter_importer_options', 'sweat_importer_set_options', 9 );
	/**
	 ...
	 */
	function sweat_importer_set_options( $options = array() ) {
		if ( is_array( $options ) ) {
			...
			// Prepare demo data
			$options['demo_url'] = esc_url( sweat_get_protocol() . ':' . sweat_storage_get( 'theme_demofiles_url' ) );
			// Required plugins
			$options['required_plugins'] = array_keys( sweat_storage_get( 'required_plugins' ) );
...
if ( ! function_exists( 'sweat_ocdi_set_options' ) ) {
	add_filter( 'trx_addons_filter_ocdi_options', 'sweat_ocdi_set_options', 9 );
	/**
	 ...
	 * @return array          Processed options with a theme-specific values.
	 */
	function sweat_ocdi_set_options( $options = array() ) {
		if ( is_array( $options ) ) {
			// Prepare demo data
			$options['demo_url'] = esc_url( sweat_get_protocol() . ':' . sweat_storage_get( 'theme_demofiles_url' ) );
			// Required plugins
...
if ( ! function_exists( 'sweat_customizer_theme_setup1' ) ) {
	add_action( 'after_setup_theme', 'sweat_customizer_theme_setup1', 1 );
	/**
	 ...
	 */
	function sweat_customizer_theme_setup1() {
		sweat_storage_set(
			'settings', array(
				...
				'allow_fullscreen'          => false,                   // Allow cases 'fullscreen' and 'fullwide' (if a next settings is true) for the body style in the Theme Options
																		...
																		// (can be removed if filter 'sweat_filter_allow_fullscreen' return false)
