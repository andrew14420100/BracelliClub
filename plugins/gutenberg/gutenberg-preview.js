...
jQuery( window ).on( 'load', function() {
	...
	if ( $editor_wrapper.length ) {
		...
		if ( typeof window.MutationObserver !== 'undefined' ) {
			...
			sweat_create_observer( 'sweat-check-visual-editor-wrapper', $editor_wrapper, function( mutationsList ) {
				sweat_gutenberg_editor_init();
			} );
		} else {
		 	sweat_gutenberg_editor_init();
		}
	...
	function sweat_gutenberg_editor_object() {
		var editor = {
			$editor: false,
			$frame: false,
			...
			if ( $editor.length ) {
				...
				if ( editor.$frame.length ) {
					if ( editor.$frame.get(0).contentDocument ) {
						...
						if ( editor.$styles_wrapper.hasClass( 'sweat_inited' ) ) {
					...
					if ( ! editor.$editor.hasClass( 'sweat_inited' ) ) {
	...
	// Init on page load
	function sweat_gutenberg_editor_init() {

		// Get Gutenberg editor object
		var editor = sweat_gutenberg_editor_object();
		if ( ! editor.$editor ) {
			return;
		}
		...
		function add_class_with_color_scheme( $obj ) {
			if ( ! $obj.hasClass( 'scheme_' + SWEAT_STORAGE['color_scheme'] ) ) {
				$obj.addClass( 'scheme_' + SWEAT_STORAGE['color_scheme'] );
			}
		}

		function add_class_with_overridden_options( $obj ) {
			for ( var i in SWEAT_STORAGE['override_classes'] ) {
				$obj.addClass( SWEAT_STORAGE['override_classes'][i].replace( '%s', SWEAT_STORAGE[ i ] ) );
			}
		}
		...
		if ( editor_type == 'post' && editor.$styles_wrapper ) {
			...
			editor.$styles_wrapper.addClass( sweat_get_class_by_prefix( editor.$editor.attr( 'class' ), 'post-type-' ) );
			...
			if ( editor.$writing_flow && SWEAT_STORAGE['sidebar_position'] != 'hide' ) {
				if ( sweat_apply_filters( 'sweat_filter_wrap_gutenberg_editor_with_sidebar', false ) ) {
		...
		if ( editor_type == 'site' && editor.$styles_wrapper ) {
			...
			add_class_with_overridden_options( editor.$styles_wrapper );
			// Mark as inited
			editor.$styles_wrapper.addClass( 'sweat_inited' );
		}

		// Widgets Editor
		//----------------------------------------------------------
		if ( editor_type == 'widgets' && editor.$writing_flow ) {
			if ( sweat_apply_filters( 'sweat_filter_add_scheme_to_widgets_editor', true ) ) {
				// Create an observer to add the class 'scheme_xxx' to the each new sidebar
				sweat_remove_observer( 'sweat-check-editor-styles-wrapper' );
				sweat_create_observer( 'sweat-check-editor-styles-wrapper', editor.$writing_flow, function( mutationsList ) {
		...
		editor.$editor.addClass( 'sweat_inited' );
	}
} );
