...
jQuery( document ).ready(
	function() {
		...
		sweat_add_filter('trx_addons_filter_sc_classes', function(classes, settings) {
		...
		setTimeout( function() {
			if ( window.elementor !== undefined ) {
				...
				jQuery( '#elementor-panel' )
					.on( 'input change', '[data-setting^="sweat_options_"]', function (e) {
						...
						var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.sweat_post_editor' );
						if ( link.length > 0 ) {
							sweat_change_post_edit_link_elementor( link );
						}
					} )
					.on( 'click', '.sweat_post_editor', function(e) {
						sweat_change_post_edit_link_elementor( jQuery(this) );
						if (jQuery(this).hasClass('sweat_hidden' )) {
							e.preventDefault();
							return false;
						}
					});
		...
		function sweat_change_post_edit_link_elementor(a) {
			if (a.length > 0) {
