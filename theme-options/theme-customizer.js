/* global sweat_color_schemes, sweat_dependencies, Color */
...
( function( api ) {
	...
	for (var i in sweat_color_schemes) {
		cssTemplate['scheme_' + i] = wp.template( 'sweat-color-scheme-' + i );
	}
	// Add Template with theme fonts
	cssTemplate['theme_fonts'] = wp.template( 'sweat-fonts' );
	// Add Template with theme vars
	cssTemplate['theme_vars'] = wp.template( 'sweat-vars' );

	// Allow an alpha channel in the color picker
	if ( sweat_customizer_vars['colorpicker_allow_alpha'] ) {
	...
	api.bind( 
		'ready', function() {
			...
			jQuery( '#customize-header-actions #save' )
				.before( '<input type="button" class="button customize-action-reset" value="' + sweat_customizer_vars['msg_reset'] + '">' );
			jQuery( '#customize-header-actions .customize-action-reset' )
				.on( 'click', function(e) {
					if (confirm( sweat_customizer_vars['msg_reset_confirm'] )) {
			...
			jQuery( '#customize-header-actions .spinner' )
				.after( '<button class="button customize-action-refresh icon-spin3" title="' + sweat_customizer_vars['msg_reload_title'] + '">' + sweat_customizer_vars['msg_reload'] + '</button>' );
			...
			if (sweat_customizer_vars && sweat_customizer_vars['theme_name_suffix']) {
				jQuery( '#customize-info .site-title' ).append( sweat_customizer_vars['theme_name_suffix'] );
			}
			...
			jQuery( '#customize-theme-controls [id^="customize-control-load_fonts"]' )
				.on( 'change', sweat_customizer_update_load_fonts );
			...
			jQuery( '#customize-theme-controls .control-section > .accordion-section-title' )
				.on( 'click', function() {
					...
					if (id !== '') {
						...
						if (section.length > 0) {
							sweat_customizer_check_dependencies( section );
						}
					}
				} );

			// Decorate subsections in the panel 'Typography'
			if ( sweat_customizer_vars['decorate_fonts'] ) {

				jQuery('body').addClass( 'sweat_decorate_fonts_section' );

				// Prepare section for open on title is clicked
				var sweat_customizer_title_clicked = function( title ) {
					var id = title.parent().attr( 'aria-owns' );
					sweat_customizer_close_opened_sections( title.parents( '#sub-accordion-panel-fonts' ), id );
					var section = jQuery( '#' + id ),
					...
					.each ( function() {
						...
						$self
							.on( 'keydown', function(e) {
								if ( e.which == 13 ) {
									sweat_customizer_title_clicked( $self );
								}
							} )
							.on( 'click', function() {
								sweat_customizer_title_clicked( $self );
				...
				var sweat_customizer_close_opened_sections = function( panel, current_id ) {
					if ( panel.length ) {
				...
				jQuery( '#customize-theme-controls #sub-accordion-panel-fonts .customize-panel-back' )
					.on( 'keydown', function(e) {
						if ( e.which == 13 ) {
							sweat_customizer_close_opened_sections( jQuery(this).parents('#sub-accordion-panel-fonts') );
						}
					} )
					.on( 'click', function() {
						sweat_customizer_close_opened_sections( jQuery(this).parents('#sub-accordion-panel-fonts') );
	...
	api.bind( 'change', function(obj) {
		...
		sweat_customizer_check_dependencies( jQuery( '#customize-theme-controls #customize-control-' + obj.id ).parents( '.control-section' ) );
		sweat_customizer_refresh_preview( obj );
	} );
	...
	api.bind( 'refresh_schemes', function() {
		...
		for (i in cssTemplate) {
			...
			i = i.replace( 'scheme_', '' );
			if (typeof sweat_color_schemes[i] === 'undefined') {
				delete cssTemplate['scheme_' + i];
				jQuery( '#tmpl-sweat-color-scheme-' + i ).remove();
			} else if (tpl_idx == '') {
				tpl_idx     = i;
				tpl_content = jQuery( '#tmpl-sweat-color-scheme-' + i ).html().trim();
			}
		}
		// Add new templates
		var regex = new RegExp( "\.scheme_" + tpl_idx, "g" );
		for (i in sweat_color_schemes) {
			if (typeof cssTemplate['scheme_' + i] === 'undefined') {
				jQuery( '#tmpl-sweat-color-scheme-' + tpl_idx )
				.clone()
				.attr( 'id', 'tmpl-sweat-color-scheme-' + i )
				.html( tpl_content.replace( regex, '.scheme_' + i ) )
				.insertAfter( jQuery( '#tmpl-sweat-color-scheme-' + tpl_idx ) );
				cssTemplate['scheme_' + i] = wp.template( 'sweat-color-scheme-' + i );
			}
		}
	} );
	...
	for (var action in sweat_customizer_vars['actions']['expand']) {
		if (action == 'length') {
			continue;
		}
		if (sweat_customizer_vars['actions']['expand'][action]['type'] == 'panel') {
			api.panel(
				action, function( panel ) {
					panel.expanded.bind(
						function( isExpanded ) {
							if ( isExpanded ) {
								var data = sweat_customizer_vars['actions']['expand'][panel.id];
			...
			api.section(
				action, function( section ) {
					section.expanded.bind(
						function( isExpanded ) {
							if ( isExpanded ) {
								var data = sweat_customizer_vars['actions']['expand'][section.id];
								if (typeof data['url'] !== 'undefined' && data['url'] !== '') {
									api.previewer.previewUrl.set( data['url'] );
								}
								if (typeof data['callback'] !== 'undefined' && data['callback'] !== '' && typeof window[data['callback']] === 'function') {
	...
	function sweat_customizer_get_field_value(fld) {
	...
	function sweat_customizer_check_dependencies(cont) {
		if ( typeof sweat_dependencies == 'undefined' || SWEAT_STORAGE['check_dependencies_now'] ) return;
			...
			function() {
				...
				if (depend) {
					...
					for (i in depend) {
						...
						if (fld.length > 0) {
							val = sweat_customizer_get_field_value( fld );
							if ( val == 'inherit' ) {
								dep_cnt = 0;
								dep_all = 1;
								var tag;
	...
	function sweat_customizer_refresh_preview(obj) {
		...
		if (updateCSS) {
			...
			if (id == 'scheme_storage') {
				...
				// Any color in the scheme_storage is changed
			} else if (Object.values( sweat_sorted_schemes ).indexOf( id ) >= 0) {
				forceUpdateCSS = true;
				...
				if (var_name !== undefined) {
					...
					sweat_customizer_update_theme_vars( jQuery( '[data-customize-setting-link="' + id + '"]' ).data( 'var_name' ), val );
					processed = true;
				}

				// Any theme fonts parameter is changed
			} else {
				for (opt in sweat_theme_fonts) {
					for (rule in sweat_theme_fonts[opt]) {
						if (opt + '_' + rule.replace( ':', '-' ) == id) {
							// Store new value to the fonts table
							sweat_customizer_update_theme_fonts( opt, rule, val );
							processed = true;
							break;
						}
					}
	...
	// Actions buttons
	function sweat_customizer_actions(e) {
	...
	function sweat_customizer_update_theme_vars(opt, value) {
		sweat_theme_vars[opt] = parseFloat( value );
	}

	// Store new value in the theme fonts
	function sweat_customizer_update_theme_fonts(opt, rule, value) {
		sweat_theme_fonts[opt][rule] = value;
	}

	// Change theme fonts options if load fonts is changed
	function sweat_customizer_update_load_fonts() {
		var opt_list = [], i, tag, sel, opt, name = '', family = '', val = '', new_val = '', sel_idx = 0;
		updateCSS    = false;
		for (i = 1; i <= sweat_customizer_vars['max_load_fonts']; i++) {
	...
	function sweat_customizer_update_css() {
		...
		var vars = sweat_customizer_add_theme_vars( sweat_theme_vars );

		// Make styles and add its to the css
		css += sweat_customizer_prepare_html_value( cssTemplate['theme_vars']( vars ) );

		// Add theme specific fonts rules
		var fonts = sweat_customizer_add_theme_fonts( sweat_theme_fonts );

		// Make styles and add its to the css
		css += sweat_customizer_prepare_html_value( cssTemplate['theme_fonts']( fonts ) );

		// Add colors
		var sorted = {}, scheme = '';
		for (var param in sweat_sorted_schemes) {
			if (param == 'length') {
				continue;
			}
			scheme = api( sweat_sorted_schemes[param] )();
			if (scheme && scheme !== 'inherit') {
				sorted[scheme] = 1;
			}
		}
		for (scheme in sweat_color_schemes) {
			sorted[scheme] = 1;
		}
		for (scheme in sorted) {
			var i, colors = [];
			// Copy all colors to the temp var
			for (i in sweat_color_schemes[scheme].colors) {
				colors[i] = sweat_color_schemes[scheme].colors[i];
			}
			// Add theme specific colors and tints
			colors = sweat_customizer_add_theme_colors( colors );
			...
			var tmpl = jQuery( '#tmpl-sweat-color-scheme-' + scheme ).html().trim();
			for (i in colors) {
	...
	function sweat_customizer_add_theme_colors(colors) {
		if (sweat_additional_colors) {
			var clr = '', v = '';
			for (var k in sweat_additional_colors) {
				v   = sweat_additional_colors[k];
				clr = colors[v['color']];
			...
			for (var css_prop in fonts[tag]) {
				...
				rez[tag + '_' + css_name] = css_value !== '' && css_value != 'inherit'
												? css_prop + ':' + ( ['font-size', 'letter-spacing', 'margin-top', 'margin-bottom', 'border-width', 'border-radius'].indexOf(css_prop) >= 0
																		? sweat_customizer_prepare_css_value( css_value )
																		: css_value
																		)
																	+ ';'
												: '';
	...
	function sweat_customizer_add_theme_vars(vars) {
		...
		if ( typeof vars['rad'] != 'undefined' ) {
			...
			rez['rad']      = sweat_customizer_prepare_css_value( vars['rad'] );
		...
		if ( typeof vars['page_width'] != 'undefined' ) {
			...
			if ( isNaN( vars['page_width'] ) || vars['page_width'] === 0) {
				vars['page_width'] = sweat_customizer_vars['page_width_default'];
			}
			vars['sidebar_width']       = parseInt( vars['sidebar_width'], 10 );
			vars['sidebar_gap']         = parseInt( vars['sidebar_gap'], 10 );

			rez['page_width']          = sweat_customizer_prepare_css_value( vars['page_width'] );
			rez['page_boxed_extra']     = sweat_customizer_prepare_css_value( vars['page_boxed_extra'] );
			rez['page_fullwide_extra']  = sweat_customizer_prepare_css_value( vars['page_fullwide_extra'] );
			rez['page_fullwide_max']    = sweat_customizer_prepare_css_value( vars['page_fullwide_max'] );
			rez['grid_gap']             = sweat_customizer_prepare_css_value( vars['grid_gap'] );
			rez['sidebar_prc']          = vars['sidebar_width'] / vars['page_width'];
			rez['sidebar_gap_prc']      = vars['sidebar_gap'] / vars['page_width'];
			rez['sidebar_width']        = sweat_customizer_prepare_css_value( vars['sidebar_width'] );
			rez['sidebar_gap']          = sweat_customizer_prepare_css_value( vars['sidebar_gap'] );
			rez['sidebar_gap_width']    = sweat_customizer_prepare_css_value( vars['sidebar_gap'] );
	...
	// Add ed to css value
	function sweat_customizer_prepare_css_value(val) {
		if (val !== '' && val != 'inherit') {
			var ed = ('' + val).slice( -1 );
			if ('0' <= ed && ed <= '9') {
				val += 'px';
	...
	function sweat_customizer_prepare_html_value(val) {
